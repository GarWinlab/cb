        ...
        if (!$curl['err']) {
            ...
            if (is_object($out) and isset($out->$key)) {
                ...
                if ($ck) {
                    ...
                    //echo "<br/> www.bitstamp.net BTC-USD ". $ck ."</br></br>";
                    //echo "www.cbr.ru USD-RUB ". $curs_parser[1]['curs2'] ."</br></br>";
                    $cm = $ck * $curs_parser[1]['curs2'];
                    //echo "www.cyber.money BTC-RUB ". $ck * $curs_parser[1]['curs2'] ."</br></br>";
                    $curs_parser[301]['curs1'] = $curs1;
                    $curs_parser[301]['curs2'] = $cm * $curs1;

                    // $wpdb->query("UPDATE `upd_naps` SET price_cron=" . $curs_parser[301]['curs2'] . " WHERE price_cron=0");

                   /* $myfile = fopen("/var/www/html/analysis/run.txt", "a") or die("Unable to open file!");
                    fwrite($myfile, "Run time is " . date("h:i:sa") .
                        " Curs  is " . $curs_parser[301]['curs2'] . "\n >>>!!!" .
                        $curs_parser[301]['curs1'] . " - " . $curs_parser[301]['curs2'] . " -=- " . $curs_parser[302]['curs1'] . " - " . $curs_parser[302]['curs2']);
                    fclose($myfile);*/

                    //rub - btc
                    $curs_parser[302]['curs1'] = 100000;
                    $temp = intval($curs_parser[301]['curs2']);//целое число
                    $btc = number_format(1 / $temp, 8, '.', '');
                    $curs_parser[302]['curs2'] = $btc * 100000;

                    $wpdb->query("UPDATE `upd_naps` SET price_cron='" . $curs_parser[301]['curs2'] . "|" . $curs_parser[302]['curs2'] . "' WHERE price_cron=0");
                }
            }
        }
    }
