...
description: [ru_RU:]мерчант WEX[:ru_RU][en_US:]WEX merchant[:en_US]
version: 1.2
*/
						...
						if($code){
							try{
								...
								if($info){
									...
									$wex_sum = is_isset($info,'amount');
									$wex_currency = strtoupper(is_isset($info,'currency'));
									$wex_trans_id = trim(is_isset($info,'trans_id'));
									if($wex_sum >= $sum){
									
										if($wex_currency == $currency){
											
											$params = array(
												'pay_purse' => '',
												'sum' => $wex_sum,
												'naschet' => '',
												'trans_in' => $wex_trans_id,
											);
