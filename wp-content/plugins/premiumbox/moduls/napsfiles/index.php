...
function getRowReport()
{
    global $wpdb;
    $item = $wpdb->get_row("SELECT * FROM `upd_report`");
    return $item;
...
function txtxml_pn_admin_content_pn_naps()
{
    ...
    $text = '<h4>Коррекция цены по localbitcoins.com и bestchange.ru </h4>' .
        '<a  href="http://cyber.money/analysis/report.html" target="_blank">http://cyber.money/analysis/report.html</a>'.
        '<form class="form" id="report-form" action="#">' .
        '<fieldset style="border:1px solid darkgrey; padding:8px;margin:8px;">' .
        '<legend>Адреса страниц:</legend>' .
        '<table style="font-size:12px">' .
        '<tr><td style="width:250px; text-align:left">localbitcoins.com BTC->RUB & RUB->BTC</td><td  style="width:100%">' .
        ' <input type="text" style="width:100%" name="link_localbitcoins" id="link_localbitcoins" value="' . $row->link_localbitcoins . '" placeholder="https://localbitcoins.com" required="">' .
        '</td></tr>' .
        '<tr><td style="width:250px; text-align:left" class="alignleft">bestchange.ru BTC->RUB</td> <td  style="width:100%">' .
        ' <input type="text" style="width:100%"  name="link_bestchange_sell" id="link_bestchange_sell" value="' . $row->link_bestchange_sell . '" placeholder="https://bestchange.ru" required="">' .
        '</td></tr>' .
        '<tr><td style="width:250px; text-align:left" class="alignleft">bestchange.ru RUB->BTC</td> <td  style="width:100%">' .
        ' <input type="text" style="width:100%"  name="link_bestchange_buy" id="link_bestchange_buy" value="' . $row->link_bestchange_buy . '" placeholder="https://bestchange.ru" required="">' .
        '</td></tr>' .
        '</table></fieldset>' .
        '<fieldset style="border:1px solid darkgrey; padding:8px;margin:8px;">' .
        '<legend >Настройки для выбора обменников:</legend>' .
        '<table style="font-size:12px">'
        .
        '<tr><td style="width:50px; text-align:left">Минимальное количество отзывов</td>' .
        '<td style="width:100%;text-align:left">' .
        ' <input type="text" size="2"  name="exchange_min_review" id="exchange_min_review"  placeholder="100" value="' . $row->exchange_min_review . '" required="">' .
        '</td>' .

        '<tr><td style="width:250px; text-align:left" class="alignleft">Минимальный резерв в рублях</td> <td  style="width:100%;text-align:left">' .
        ' <input type="text" size="5" style="" id="reserv_min_rub" name="reserv_min_rub" value="' . $row->reserv_min_rub . '" placeholder="700000" required="">' .
        '</td></tr>' .
        '<tr><td style="width:250px; text-align:left" class="alignleft">Минимальный резерв в биткойнах</td> <td  style="width:100%;text-align:left">' .
        ' <input type="text" size="1" style=""  name="reserv_min_btc" id="reserv_min_btc" value="' . $row->reserv_min_btc . '" placeholder="2" required="">' .
        '</td></tr>' .
        '</table></fieldset>' .
        '<fieldset style="border:1px solid darkgrey; padding:8px;margin:8px;">' .
        '<legend >Коррекция:</legend>' .
        '<table style="font-size:12px">' .
        '<tr><td class="alignleft" style="width:80px; text-align:left; text-wrap: none">Шаг в руб.</td><td  style="width:100%;text-align:left"">' .
        ' <input type="text" size="2" name="step_size_rub" id="step_size_rub" value="' . $row->step_size_rub . '" placeholder="1000" required="">' .
        '</td></tr>' .
        '<tr><td class="alignleft" style="width:80px; text-align:left; text-wrap: none">Шаг в %</td><td  style="width:100%;text-align:left"">' .
        ' <input type="text" size="2" style="" name="step_size_percent" id="step_size_percent" value="' . $row->step_size_percent . '" placeholder="0" required="">' .
        '</td></tr>' .
        '</table></fieldset>' .
        '<button type="button" onclick="$.saveReport()" class="button button-primary alignleft">Сохранить</button>' .
        ' </form><div id="report_resp"></div><br/><br/>';
...
function napsfiles_tab_naps_tab12($data, $data_id)
{
    ...
    <tr>
        ...
        <td colspan="2">
            <div class="premium_wrap_standart">
                <input type="text" name="xml_city" style="width: 200px;"
                       value="<?php echo pn_strip_input(is_isset($data, 'xml_city')); ?>"/>
            </div>
        </td>
    </tr>
    <tr>
